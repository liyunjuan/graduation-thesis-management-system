...
@Service
public class PaperScoreServiceImpl implements PaperScoreService<PaperScore> {
    @Autowired
    private PaperScoreDAO paperScoreDAO;
    @Autowired
    private PaperTitleDAO paperTitleDAO;

    public boolean save(PaperScore obj) {
        try {
            this.paperScoreDAO.save(obj);
            return true;
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public boolean update(PaperScore obj) {
        try {
            this.paperScoreDAO.merge(obj);
            return true;
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public PaperScore findById(int id) {
        try {
            return this.paperScoreDAO.findOne(id);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
    ...
    public boolean del(PaperScore obj) {
        try {
            this.paperScoreDAO.delete(obj);
            return true;
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public PaperScore findScoreByTitleId(int id) {
        try {
            ...
            if ((scoreList == null) || (scoreList.size() == 0)) {
                PaperTitle t = this.paperTitleDAO.findOne(id);
                if (t == null) {
                    return null;
                }
    ...
    public int teacherInput(int titleId, String comment, double score) {
        try {
            ...
            if ((scoreList == null) || (scoreList.size() == 0)) {
                ...
                PaperTitle t = this.paperTitleDAO.findOne(titleId);
                sc.setPaperTitle(t);
                scoreList.add(sc);
            }
    ...
    public int councilInput(int titleId, String comment) {
        try {
            ...
            if ((scoreList == null) || (scoreList.size() == 0)) {
                PaperScore sc = new PaperScore();
                PaperTitle t = (PaperTitle) this.paperTitleDAO.findOne(titleId);
                sc.setPaperTitle(t);
                scoreList.add(sc);
            }
            PaperScore sco = (PaperScore) scoreList.get(scoreList.size() - 1);
