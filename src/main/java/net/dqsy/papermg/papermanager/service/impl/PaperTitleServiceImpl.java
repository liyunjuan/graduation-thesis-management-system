...
@Service
public class PaperTitleServiceImpl implements PaperTitleService<PaperTitle> {
    ...
    @Override
    public String savePaperTitle(PaperTitle paperTitle) {
        try {
            int flag = findByProperty("paperTitleKeywords", paperTitle.getPaperTitleKeywords(), 1, 1).getList().size();
            if (flag != 0) {
                return "检测到关键字相同,请检查后重新填写！";
            }
            flag = findByProperty("paperTitleName", paperTitle.getPaperTitleName(), 1, 1).getList().size();
            if (flag != 0) {
                return "检测到相同题目与名称,请不要重复提交！";
            }
            PaperTitleState paperTitleState = this.paperTitleStateService.findByState(1);
    ...
    public String saveStudentDTP(int studentId, int teacherId, PaperTitle paperTitle) {
        try {
            int flag = findByProperty("paperStudent.studentId", studentId, 1, 1).getList().size();
            if (flag != 0){
    ...
    @Override
    public boolean save(PaperTitle obj) {
        try {
            paperTitleDAO.save(obj);
            return true;
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public boolean update(PaperTitle ojb) {
        try {
            this.paperTitleDAO.merge(ojb);
            return true;
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public PaperTitle findById(int id) {
        try {
            return this.paperTitleDAO.findOne(id);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public PagingSupport findByProperty(String property, String value, int numberOfPage, int countOfPage) {
        try {
            return this.paperTitleDAO.findByProperty("PaperTitle", property, value, numberOfPage, countOfPage);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
    ...
    public List findTeacherName() {
        try {
            return this.paperTitleDAO
                    .find("select new net.dqsy.papermg.papermanager.vo.TeacherNameVO(paperTeacher.teacherId,paperTeacher.teacherName) from PaperTitle where paperTitleState.paperTitleState = 1 group by paperTeacher.teacherName",
                            null, 1, 9999).getList();
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public List findTitleByStudentId(int paperStudentId, int state) {
        try {
            return this.paperTitleDAO
                    .find("select new net.dqsy.papermg.papermanager.vo.PaperTitleVO(paperTitleId,paperTitleState.paperTitleState,paperTitleName,paperTeacher.teacherName) from PaperTitle where paperTitleState.paperTitleState >" +
                            state +
    ...
    public List findTitleByTeacherId(int paperTeacherId, int state) {
        try {
            return this.paperTitleDAO
                    .find("select new net.dqsy.papermg.papermanager.vo.PaperTitleVO(paperTitleId,paperTitleName,paperTitleState.paperTitleState) from PaperTitle where paperTitleState.paperTitleState > " +
                            state +
    ...
    public PagingSupport findByTeacherId(int paperTeacherId, int numberOfPage, int countOfPage) {
        try {
            return this.paperTitleDAO.findByProperty("PaperTitle", "paperTeacher.teacherId", paperTeacherId, numberOfPage, countOfPage);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
    ...
    public PagingSupport findAll(int numberOfPage, int countOfPage) {
        try {
            return this.paperTitleDAO.findAll("PaperTitle", numberOfPage, countOfPage);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
    ...
    public boolean del(PaperTitle obj) {
        try {
            this.paperTitleDAO.delete(obj);
            return true;
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public boolean updateState(int paperTitleId, int state) {
        try {
            PaperTitle paperTitle = findById(paperTitleId);

            PaperTitleState paperTitleState = this.paperTitleStateService.findByState(state);
            paperTitle.setPaperTitleState(paperTitleState);
            ...
            this.paperTitleDAO.merge(paperTitle);
            return true;
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public boolean updateStateByBatch(List<Integer> paperTitleIds, int state) {
        for (int i = 0; i < paperTitleIds.size(); i++) {
            if (!updateState((paperTitleIds.get(i)).intValue(), state))
                return false;
        }
        return true;
    }
