...
@Service
public class PaperStudentServiceImpl implements PaperStudentService<PaperStudent> {
    @Autowired
    ...
    public boolean save(PaperStudent obj) {
        try {
            this.logger.info("saving PaperStudent instance");

            PaperStudent paperStudent = obj;
            ...
            if (this.paperUserService.save(paperUser)) {
                ...
                paperUser = (PaperUser) this.paperUserService
                        .findByProperty("userName", paperStudent.getStudentNumber(), 1, 1).getList().get(0);

                PaperRole paperRole =
                        (PaperRole) this.paperRoleService.findByProperty("roleName", "学生", 1, 1).getList().get(0);
    ...
    public boolean update(PaperStudent obj) {
        try {
            this.paperStudentDAO.merge(obj);
        } catch (PaperManagerException e) {
            e.printStackTrace();
            return false;
        }
    ...
    public PaperStudent findById(int id) {
        try {
            return this.paperStudentDAO.findOne(id);
        } catch (PaperManagerException e) {
            e.printStackTrace();
        }
        return null;
    ...
    public PagingSupport findByProperty(String property, String value, int numberOfPage, int countOfPage) {
        try {
            return this.paperStudentDAO.findByProperty("PaperStudent", property, value, numberOfPage, countOfPage);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
    ...
    public PagingSupport findAll(int numberOfPage, int countOfPage) {
        try {
            return this.paperStudentDAO.findAll("PaperStudent", numberOfPage, countOfPage);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
    ...
    public boolean del(PaperStudent obj) {
        try {
            this.paperStudentDAO.delete(obj);
        } catch (Exception e) {
            e.printStackTrace();
            return false;
        }
    ...
    public PaperStudent findByNumber(String number) {
        try {
            ...
            List list = this.paperStudentDAO.find(hql, map, 1, 1).getList();
            if (list.size() == 1){
